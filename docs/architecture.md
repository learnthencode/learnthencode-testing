...
- **Interactions** — `src/assertions/react/fire.js` simulates clicks, typing, selection, submit, and reset inside `act()`. Controlled inputs follow React 19's change-event machinery: a `focusin` dispatch makes the input React's active element, the native value setter updates the DOM value, and a `keydown` dispatch triggers the change handler through the input-event polyfill. jsdom's missing `attachEvent`/`detachEvent` (used by React's value-change watcher) is shimmed onto `HTMLElement.prototype` in `environment.js`. Since v1.3.2 every interaction subtype (`click`, `type`, `change`, `select`, `submit`, `reset`) accepts `check.fetch` mocks (installed before render) and settles pending timers/fetch responses after the interaction, so UI updates driven by `POST`/`PUT`/`DELETE` responses are committed before expectations run. `verifyExpect` (`src/assertions/react/interactions.js`) evaluates multiple expectations against the whole container — `hasNoText`, `missingItem`, `text`, `hasItem`, `selector`+`value`, `selector`+`checked` — while the legacy single-rule `selector`+`text` form stays an exact element-text match.
- **Fetch mocking** — `src/assertions/react/fetch.js` intercepts `window.fetch` for the sandbox. Mocks are installed *before* `renderComponent` so `useEffect` fetches issued during the render flush hit the mock. Since v1.3.2 route keys may be method-prefixed (`"POST /api/users"`); `resolveFetchMock(mocks, url, method)` resolves each request by trying, in order: the best method-prefixed route for the request method (longest URL substring), an exact plain URL key, the longest plain URL substring, then the `"*"` catch-all. Scenarios are `success`/`error`/`empty`/`loading`; unmatched URLs resolve to a successful empty response so components render their fallback state. Every call is recorded on `engine.fetchCalls` as `{ url, method, headers, body }` (`body` stays the raw string, `null` when absent), and `parseMethodRouteKey`/`fetchCallMatches` (`src/assertions/react/fetch.js`) match recorded calls back to mock keys — plain keys match any method, method-prefixed keys require both.
...
React assertions (v1.3.0, source analysis v1.3.1, CRUD v1.3.2) use `type: "react"` and dispatch to `src/assertions/react/index.js` by `check.subtype` (43 subtypes covering render checks, interactions, effects/fetch, routing, static source analysis, and CRUD request verification). Validation lives in `validateReactRequirement` (`src/core/validate-requirement.js`), which shares the field conventions of the other types.
...
v1.3.2 adds CRUD support for the React API labs: method-aware fetch mocking, request recording, request-inspecting assertions, list content checks, and multi-expectation interactions.

- **Method-aware mocking** — `src/assertions/react/fetch.js` parses method-prefixed route keys (`"POST /api/users"`) with `parseMethodRouteKey` (a regex anchored at the start of the key; `"HEAD /api"` style unknown verbs are rejected at validation, never silently treated as URLs). `resolveFetchMock(mocks, url, method)` implements the precedence: method-prefixed route for the request method with the longest URL substring, then exact plain URL key, then longest plain URL substring, then `"*"`. `fetchCallMatches(call, key)` applies the same semantics when matching recorded calls against route keys.
- **Request recording** — the sandbox `fetch` in `src/core/react-engine.js` records `{ url, method, headers, body }` (raw body string, `null` when absent) on `engine.fetchCalls` before resolving the mock, so request assertions never depend on what the mock served.
- **Request assertions** — `src/assertions/react/fetch-assertions.js` adds `method` and `requestBody`. Both may trigger the request via `check.selector` (clicks the first match, e.g. an edit/delete button) and/or `check.values` (fills the matching fields and submits the enclosing form, e.g. a create form). `method` requires `check.expect.method` (GET/POST/PUT/PATCH/DELETE) and `check.expect.url`; `requestBody` deep-equals the parsed body against `check.expect` via `parseRequestBody` (JSON strings decoded; non-JSON bodies compared raw, so string bodies need no quoting). Failure messages list every recorded request (`METHOD url with body ...`) so learners see what their code actually sent.
- **List content assertions** — `src/assertions/react/render-assertions.js` adds `hasItem`, `missingItem`, and `hasNoText`, sharing one `listContentAssertion(engine, requirement, kind)` helper. Each renders with `check.fetch` mocks installed, settles, then checks the whole container: `hasItem` requires the text, `missingItem`/`hasNoText` require its absence (any occurrence fails).
- **Multi-expectation interactions** — `src/assertions/react/interactions.js` reworked `verifyExpect` to evaluate several rules per interaction (see the engine section above), `installFetchMocks`/`settle` (up to eight macro-task rounds) wired into all six interaction subtypes, and exported `formForValues`/`fillAndSubmitValues` so `click`/`submit` can drive form submission.
- **Validation** — `src/core/validate-requirement.js` registers the five new subtypes in `REACT_ASSERTION_SUBTYPES` (all join `COMPONENT_REQUIRED_SUBTYPES`); `hasItem`/`missingItem`/`hasNoText` require `text`; `method` requires the method/url expect; `requestBody` requires a non-empty object expect; `validateFetchRoutes` rejects unknown method prefixes, non-number `status`, and invalid `scenario` values; interaction subtypes validate optional `check.fetch`.
- **Tests** — `tests/react-fetch-methods.test.js`, `tests/react-crud-assertions.test.js`, `tests/react-crud-validation.test.js`, and `tests/react-crud-integration.test.js` cover route resolution, request matching, assertion pass/fail messages, validation, and full-runner labs (a 9-requirement passing lab and a 3-requirement failing lab). `CRUD_USERS_APP_JSX` in `tests/helpers/react-lab-fixtures.js` is the shared full-CRUD fixture (GET list, POST create, PUT edit via `[data-action='edit']` + `#edit-name`, DELETE via `[data-action='delete']`). A runnable example lives in `examples/react-crud/` (16 requirements, all passing).
